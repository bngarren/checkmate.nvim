...
|--------------|-------------|
| `archive` | Archive all completed todo items in the buffer. This extracts them and moves them to a bottom section. See api `archive()` |
...
| `lint` | Lint this buffer for Checkmate formatting issues. Runs automatically on `InsertLeave` and `TextChanged`. See api `lint()` |
