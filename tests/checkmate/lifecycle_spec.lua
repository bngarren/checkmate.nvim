  ...
  describe("plugin state", function()
    ...
    it("should clean up resources on stop", function()
      ...
      vim.wait(50, function()
        return buffer._local:get("setup_complete") == true
      end, 10)
...
- [ ] Todo B
  ...
  Regular line
      ...
      assert.is_false(checkmate._is_running())
      assert.is_false(Buffer.is_active(bufnr))
      assert.equal(0, Buffer.count_active())
  ...
  describe("buffer activation", function()
    ...
    it("should handle buffer deletion", function()
      ...
      local active_buffers = Buffer.get_active_buffer_map()
  ...
  pending("should handle configuration changes while running", function()
    ...
    local buffer1 = require("checkmate.buffer").get(buf1)

    vim.cmd("edit tasks.md")
    local buf2 = vim.api.nvim_get_current_buf()
    vim.bo[buf2].filetype = "markdown"

    local buffer2 = require("checkmate.buffer").get(buf2)

    assert.is_true(buffer1._local:get("setup_complete"))
    assert.is_falsy(buffer2._local:get("setup_complete"))
    ...
    assert.is_falsy(buffer1._local:get("setup_complete"))
    assert.is_true(buffer2._local:get("setup_complete"))
